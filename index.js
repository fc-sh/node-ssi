...
function resolve(tpl) {
  ...
  if (lastMatches) {
    fnStr +=
      '_r+="' +
      resolveLine(tpl.slice(lastMatches.index + lastMatches[0].length)) +
      '";';
  } else {
...
SSI.prototype = {
  ...
  resolveIncludes: function (content, options, callback) {
    ...
    async.whilst(
      // https://www.npmjs.org/package/async#whilst-test-fn-callback-
      function test() {
      ...
      function insertInclude(next) {
        ...
        basePath =
          (options.dirname && RegExp.$3.charAt(0) !== "/") || isVirtual
            ? options.dirname
            : options.baseDir;
        tpath = path.join(basePath, RegExp.$3);
        fs.lstat(tpath, function (err, stats) {
          if (err) {
            return next(err);
          }
            ...
            function (err, innerContentRaw) {
              ...
              ssi.resolveIncludes(
                ...
                function (err, innerContent) {
                  ...
                  content =
                    content.slice(0, matches.index) +
                    innerContent +
                    content.slice(matches.index + seg.length);
                  next(null, content);
                }
              );
            }
